...
# run_benchmark.sh - Reusable benchmark runner for LIO-EKF (ROS1)
set -e
...
echo "=========================================="
echo "Running LIO-EKF benchmark (ROS1)"
...
IMAGE="${LIOEKF_IMAGE:-ghcr.io/mapshd/lioekf2hdmapping:latest}"
...
docker run --rm -t \
    ...
    bash -lc "
      ...
      roslaunch ${LIOEKF_LAUNCH} >/tmp/lioekf.log 2>&1 &
      LAUNCH_PID=\$!
      ...
      rosrun lio-ekf-to-hdmapping listener \"\${REC_BAG}\" \"\${OUT_DIR}/hdmapping\"
    "
